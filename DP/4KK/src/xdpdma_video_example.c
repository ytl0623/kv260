...
u8 Frame[BUFFERSIZE] __attribute__ ((__aligned__(256))); // 1920x1080, 32-bit // align address
...

{
	...
	// where, how?
	FrameBuffer.Address = (INTPTR)Frame; // start address
	FrameBuffer.Stride = STRIDE; // 1920x4(Byte)
	FrameBuffer.LineSize = LINESIZE; // LineSize = Stride
	FrameBuffer.Size = BUFFERSIZE; // 1920x1080x4(Byte)
...
//		RunCfgPtr->VideoMode = XVIDC_VM_3840x2160_60_P;
...
u8 *GraphicsOverlay(u8* Frame, Run_Config *RunCfgPtr)
{
	u64 Index;
	u32 *RGBA;
	RGBA = (u32 *) Frame; // unsigned 8-bit -> 32-bit
	/*
		 * Red at the top half
		 * Alpha = 0x0F
		 * */
