...
void ConfigVtc(XVidC_VideoStream *StreamPtr) {
	...
	vtc_timing.HFrontPorch = StreamPtr->Timing.HFrontPorch / PixelsPerClock; // 88
	vtc_timing.HSyncWidth = StreamPtr->Timing.HSyncWidth / PixelsPerClock; // 44
	vtc_timing.HBackPorch = StreamPtr->Timing.HBackPorch / PixelsPerClock; // 148
	vtc_timing.HSyncPolarity = StreamPtr->Timing.HSyncPolarity; // 1

	vtc_timing.VActiveVideo = StreamPtr->Timing.VActive; // 1080
	vtc_timing.V0FrontPorch = StreamPtr->Timing.F0PVFrontPorch; // 4
	vtc_timing.V0SyncWidth = StreamPtr->Timing.F0PVSyncWidth; // 5
	vtc_timing.V0BackPorch = StreamPtr->Timing.F0PVBackPorch; // 36
	vtc_timing.VSyncPolarity = StreamPtr->Timing.VSyncPolarity; // 1

	xil_printf("--- VTC Generator Timing Parameters ---\n\r");
	xil_printf("Horizontal Timing (in Pixels/Clocks):\n\r");
	xil_printf("  H Active Video:  %d\n\r", vtc_timing.HActiveVideo);
	xil_printf("  H Front Porch:   %d\n\r", vtc_timing.HFrontPorch);
	xil_printf("  H Sync Width:    %d\n\r", vtc_timing.HSyncWidth);
	xil_printf("  H Back Porch:    %d\n\r", vtc_timing.HBackPorch);
	xil_printf("  H Sync Polarity: %d\n\r", vtc_timing.HSyncPolarity);
	xil_printf("\n\r");
	xil_printf("Vertical Timing (in Lines):\n\r");
	xil_printf("  V Active Video:  %d\n\r", vtc_timing.VActiveVideo);
	xil_printf("  V Front Porch:   %d\n\r", vtc_timing.V0FrontPorch);
	xil_printf("  V Sync Width:    %d\n\r", vtc_timing.V0SyncWidth);
	xil_printf("  V Back Porch:    %d\n\r", vtc_timing.V0BackPorch);
	xil_printf("  V Sync Polarity: %d\n\r", vtc_timing.VSyncPolarity);
	xil_printf("-------------------------------------\n\r");
